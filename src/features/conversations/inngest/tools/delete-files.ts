...
export const createDeleteFilesTool = ({
  ...
  return createTool({
    name: "deleteFiles",
    description:
      "Delete files or folder from the project. If deleting a folder, all contents will be deleted recursively.",
    parameters: z.object({
      fileIds: z
        .array(z.string())
    ...
    handler: async (params, { step: toolStep }) => {
      ...
      try {
        ...
        return `Error deleting files or folders: ${error instanceof Error ? error.message : "Unknown error"}`;
      }
    },
  });
};
